...
**Startup sequencing — Background backfill for fast boot.** The initial backfill and `RepairAnalytics` pass run in a background goroutine, allowing the HTTP server to boot instantly and pass health checks immediately. This follows standard microservice patterns, ensuring that a container orchestrator (like Kubernetes) won't kill the process if the external API is slow or rate-limited. Clients can use the `GET /sync/status` endpoint to monitor the backfill progress. The `RepairAnalytics` pass catches the failure mode where the server crashed between `SaveSyncResult` (data committed) and `ComputeAll` (analytics written) — without this, a scheme could be permanently stuck with synced NAV data but no analytics.
